...
<style>
.rotary { position: relative; width: 854px; height: 504px; margin: 0 auto; background: #d71f2e url(/Public/images/bg11.png);}
.rotaryArrow { position: absolute; left: 181px; top: 104px; width: 294px; height: 294px; cursor: pointer; background-image: url(/Public/images/arrow.png);}

.list { position: absolute; right: 48px; top: 144px; width: 120px; height: 320px; overflow: hidden;}
.list h3 { display: none;}
.list ul { list-style-type: none;}
.list li { height: 37px; font: 14px/37px "Microsoft Yahei"; color: #ffea76; text-indent: 25px; background: url(/Public/images/user.png) 0 no-repeat;}
...
.result a { position: absolute; right: 5px; top: 5px; width: 25px; height: 25px; text-indent: -100px; background-image: url(/Public/images/close.png); overflow: hidden;}
...
<script src="/Public/js/jquery.rotate.min.js"></script>
<script>
$(function(){
	...
	$rotaryArrow.click(function(){
		...
		var url="http://www.new.com/Public/Demo/?service=Jiang.jiang";
		var reqdata={order_id:"<?php echo ($order_id); ?>",user_id:"<?php echo ($uid); ?>"};
		$.post(url,reqdata,function(jsonobj){
			if(jsonobj.data.code=='1'){
				$resultTxt.html(jsonobj.data.content);
				$result.show();
				return false;
			}else if(jsonobj.data.code=='0'){
				data = parseInt(jsonobj.data.item);
				switch(data){
					case 1: 
